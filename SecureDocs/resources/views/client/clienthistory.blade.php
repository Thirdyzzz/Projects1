                    ...
                    <div class="bg-white rounded-lg shadow-xl mt-4 px-8">
                        <div class="bg-white dark:bg-white-100 overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-4 border-b">
                                ...
                                <form action="{{route('clientviewhistory', $clientData)}}">
                                    <div class="grid grid-cols-2 gap-2 align-items-center justify-items-center">
                                        ...
                                        <div class="pl-20">
                                            ...
                                            <button type="submit"
                                                class="bg-blue-500 text-white rounded p-1 pl-2 pr-2">Filter</button>
                                        </div>
                                    </div>
                                </form>
                                <div>
                                    ...
                                    <table id="dataTable" class="table-auto border-collapse w-full">
                                        ...
                                        <tbody id="tableBody" class="text-sm font-normal text-gray-700">
                                            <tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
                                                <td class="px-4 py-4">{{$billingRecord->id}}</td>
                                                <td class="px-4 py-4">{{$billingRecord->case_id}}</td>
                                                <td class="px-4 py-4">{{$billingRecord->case_title}}</td>
                                                <td class="px-4 py-4">{{$billingRecord->b_clientbalance !== null &&
                                                    $billingRecord->b_clientbalance > 0 ? 'PHP ' .
                                                    $billingRecord->b_clientbalance : 'PHP 0' }}</td>
                                                <td class="px-4 py-4">{{$billingRecord->b_casetype}}</td>
                                                <td class="px-4 py-4">{{$billingRecord->b_paymethod}}</td>
                                                <td class="px-4 py-4">{{$billingRecord->bclient_paystatus}}</td>
                                                <td class="px-4 py-4">{{$billingRecord->created_at}}</td>
                                            </tr>
                                            @endforeach
                                ...
                                <div id="pagination"
                                    ...
                                    {{ $billingRecords->appends(['search' => request('search'), 'sort_by' =>
                                    request('sort_by'),'date_from' => request('date_from'), 'order' => request('order')])->links() }}
                                </div>
                            </div>
